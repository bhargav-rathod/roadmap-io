    ...
    try {
      ...
      ${roadmapData.role} at ${roadmapData.company},`;
        ...
        prompt += `The roadmap should be highly structured and tailored to real-world expectations, including the following sections:\n\n\
                    ...
                    3. Interview Questions - Include over 150+ recently asked interview questions across all rounds.\n`;
        ...
        if (roadmapData.roleType === 'IT') {
            prompt += `\n (If DSA is required and asked in the interview then only add this point 5) 5. DSA & System Design Questions - Include at least **150+ Data Structures and Algorithms (DSA)** and **System Design** detailed questions (No Answers).`;
        ...
        if (roadmapData.includeOtherDetails && roadmapData.otherDetails.trim() !== '') {
            prompt += `\n8. Additional Instructions For Roadmap Creation - ${roadmapData.otherDetails}\n`;
        }

        prompt += `\nMake sure the roadmap is clear, long, structured, and practically applicable.`
        
        //prompt += `Include bullet points, sections, and formatting where necessary (skip the title).`;
        prompt += `Give the complete content in JSON structure only.`;
